...
vira-own [l_func ex_dist danger danger_anti]
globals [func_virus virus_antigene n_antigene n_virus spm_virus rwm_virus pnl_virus A_eq_virus a_norm_virus a_matrix_virus]
...
to reset
  set A_eq_virus matrix:make-constant 1 n_virus 0
  set a_norm_virus matrix:make-constant 1 n_virus 0
  let abc choose n_virus 0 func_virus A_eq_virus a_norm_virus a_matrix_virus
end
...
to setup
  ...
  set func_virus ["move-right" "move-left" "move-up" "move-down"]
  set virus_antigene ["left" "right" "down" "up"]
  set n_antigene 4
  set n_virus 4
  set spm_virus matrix:make-constant n_virus n_antigene 0
  set rwm_virus matrix:make-constant n_virus n_antigene 0
  set pnl_virus matrix:make-constant n_virus n_antigene 0
  set A_eq_virus matrix:make-constant 1 n_virus 0
  set a_norm_virus matrix:make-constant 1 n_virus 0
  set a_matrix_virus matrix:make-constant n_virus n_virus 0
 ...
 ; pre-learn virus_antigene func_virus spm_virus rwm_virus pnl_virus
  ...
  ask vira [set l_func -1]

  create-antibodies 20 [set color pink]
...
to go
  ask antibodies [move]
  ask vira [let antigene (position find-relative-position virus_antigene)
            ...
            set l_func choose n_virus antigene func_virus A_eq_virus a_norm_virus a_matrix_virus
            if antigene != false [;reset
                                  set l_func choose n_virus antigene func_virus A_eq_virus a_norm_virus a_matrix_virus
                                  act l_func func_virus]
...
to-report choose [n antigene func A_eq a_normalised a_matrix]
  ...
  if n > 0[
    ...
    while [i < n] [let j 0
                   ...
                   matrix:set A_eq 0 i ( ( (alpha * (1 / (n - 1)) * stimul_sum) - (alpha * (1 / (n - 1)) * inhib_sum) + (beta * direct_stimul) - natural_death ) * (matrix:get A_normalised 0 i) + (matrix:get A_eq 0 i) )
