...
vira-own [energy last_energy split_time edible l_func own_a_eq own_a_norm spm_virus rwm_virus pnl_virus a_matrix_virus n_virus n_antigene virus_antigene func_virus antigene_learn]
...
to split-vira
  if split_time <= 0 and energy > 2;and [pcolor] of patch-here = black
  [
    hatch 1
    [ set split_time (random vira_reproduction_countdown)
      set energy (energy / 2 - 1) ]
    set energy (energy / 2 - 1)
    ;ask patch-here
    ;[ set pcolor grey set growth_time cells_respawn ]
  ]
  if energy <= 2 [eat-cells]
end
...
to eat-vira
  ...
  let eaten? false
  if any? vira-here
  [ ask one-of vira-here [if-else edible [die set eaten? true][set edible true]]]

  if eaten?
  [ set energy (energy + (random vira_bonus))
    show virus_eaten
    set virus_eaten (virus_eaten + 1)
    show virus_eaten
    if random-float 1 > 0.98 [split] ]
end
...
to memorize [n specific_matrix reward_matrix penalty_matrix chosen antigene a_matrix]
  ...
  let i 0
  while [i < n] [let j 0
                 while [j < n] [if (matrix:get specific_matrix i antigene + (matrix:get specific_matrix j antigene) ) != 0 and i != j [matrix:set a_matrix i j ( ((matrix:get penalty_matrix i antigene) + (matrix:get reward_matrix j antigene)) / ((matrix:get specific_matrix i antigene) + (matrix:get specific_matrix j antigene)))]
                                set j (j + 1)
                                ]
                 set i (i + 1)
                 ]
...
to choose-strategy
  ...
  [ ask one-of vira
    [ if countdown_i = 0
      ...
      if ticks = 0 [ set lastc choose n_mode antigene_mode func_mode a_eq_mode a_norm_mode a_matrix_mode alpha_mode beta_mode natural_death_mode ]
...
to-report compute-antigene-strategy
  ...
  let virus_death (virus_eaten / virus_number)
  ;let cells_death (cells_eaten / cells_number)
  ...
  show virus_death
  if virus_death > 0.1 [report 2]
  if vn <= cn * 0.2 [report 0]
  if vn > cn * 0.2 [report 1]
...
to-report duplicate-learn
  report rmd_virus <= (count vira) and (count vira) < 200
end

to-report expansion-learn
  report rmd_cells - (count patches with [pcolor = black]) > 0
end
...
"" ""
PENS
"duplication" 1.0 0 -16777216 true "" "plot matrix:get a_norm_mode 0 0"
"expansion" 1.0 0 -7500403 true "" "plot matrix:get a_norm_mode 0 1"
"reduction" 1.0 0 -2674135 true "" "plot matrix:get a_norm_mode 0 2"
